...
from models.mlp_policy import Policy  # Actor network; note that we only need the actor for evaluation
...
def eval_loop_model(num_episodes, env_cls, env_config, conf):
    ...
    model_save_path = to_absolute_path(os.path.join(os.path.dirname(__file__), 'actor critic', 'actor_critic_model.pth'))
    ...
    hidden_dim = conf["agent_config"]["actor_critic_params"]["hidden_dim"]
    actor = Policy(state_dim, action_dim, [hidden_dim])
